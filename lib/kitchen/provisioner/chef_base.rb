...
module Kitchen
  ...
  module Provisioner
    ...
    class ChefBase < Base
      ...
      # Appends the driver's cache directory to
      # +config[:chef_omnibus_install_options]+, unless {#omnibus_dir_option}
      # was already passed there by the user.
      #
      # Mutates the provisioner's config.
      #
      # @return [void]
      # @api private
      ...
      # Loads the cookbook dependency resolver library the project needs.
      #
      # A Policyfile wins over a Berksfile; when neither exists no resolver
      # library is loaded at all.
      #
      # @return [void]
      # @raise [Kitchen::UserError] if the chosen resolver's gem is missing
      # @api private
      ...
      # Shell code that stages an install script on the instance and runs it.
      #
      # The generated installer is written to
      # +<root_path>/chef-installer.sh+, made executable, and run under sudo.
      # This is Bourne shell only -- it is reached solely from the
      # non-PowerShell branch of {#script_for_product}.
      #
      # @param command [String] the install script generated by mixlib-install
      # @return [String] Bourne shell code
      # @api private
      ...
      # Writes the rendered client.rb/solo.rb into the sandbox directory.
      #
      # The idempotency-check config is written alongside it when
      # +enforce_idempotency+ is set.
      #
      # @return [void]
      # @api private
      ...
      # Writes a second config file, plus the report handler it loads, into the
      # sandbox directory so a converge that still updates resources fails.
      #
      # @param data [Hash] the rendered config data assembled by
      #   {#prepare_config_rb}
      # @return [void]
      # @api private
      ...
      #   should read, relative to the instance's root path
      # @return [Array<String>] an array of command line arguments
      # @raise [RuntimeError] always, unless a subclass overrides this
      # @api private
      ...
      # Joins every converge into a single shell command.
      #
      # On Windows the commands are chained with an explicit +$LastExitCode+
      # check, because PowerShell does not stop on a failed native command;
      # Bourne shells use +&&+.
      #
      # @param base_cmd [String] the chef executable invocation to repeat
      # @return [String] one shell command that runs every converge
      # @api private
      ...
      # Builds one wrapped chef command per configured converge.
      #
      # +multiple_converge+ commands are emitted. Only the last one carries the
      # exit-code suffix, and it reads +client_no_updated_resources.rb+ instead
      # of the normal config when +enforce_idempotency+ is set.
      #
      # @param base_cmd [String] the chef executable invocation to repeat
      # @return [Array<String>] one shell command per converge
      # @api private
      ...
      # Concatenates the chef invocation with its arguments and wraps the
      # result in the platform's shell-specific preamble.
      #
      # @param base_cmd [String] the chef executable invocation
      # @param configfile [String] basename of the config file handed to
      #   {#chef_args}
      # @param append [String, nil] trailing shell fragment, such as
      #   {#last_exit_code}
      # @return [String] wrapped, prefixed shell code for a single converge
      # @api private
